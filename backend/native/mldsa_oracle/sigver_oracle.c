...
int main(int argc, char **argv)
{
  ...
  if (argc != 4)
  {
    fprintf(stderr, "usage: %s <pk_hex> <message_hex> <signature_hex>\n",
            argv[0]);
    return 2;
  }

  if (parse_hex("pk", argv[1], pk, sizeof(pk)) != 0)
  {
    return 2;
  }
  ...
  if (parse_hex("signature", argv[3], sig, sizeof(sig)) != 0)
  {
    free(message);
    return 2;
  }
   ...
   * message input, empty prefix, and externalmu=0.
   */
  rc = mldsa_verify_internal(sig, sizeof(sig), message, message_len, NULL, 0, pk,
                             0);
  free(message);
