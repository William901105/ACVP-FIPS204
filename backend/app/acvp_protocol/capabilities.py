...
SCHEMA_ACCEPTED_BUT_NOT_GENERATED_HASH_ALGS = ["SHAKE-128", "SHAKE-256"]
...
_SCHEMA_ACCEPTED_NOT_GENERATED_HASH_SET = set(SCHEMA_ACCEPTED_BUT_NOT_GENERATED_HASH_ALGS)

NEXT_VECTOR_GENERATION_ACTION = (
    "Server-side vector generation from negotiated capabilities is planned for Phase 3-4."
)
SHAKE_NOT_GENERATED_REASON = (
    "Accepted by schema but not generated by local Python preHash oracle because "
    "SHAKE output length is not represented in the current API."
)
...
def _negotiate_hash_algs(
    ...
    for value in values:
        if value in _SUPPORTED_HASH_ALG_SET and value not in negotiated:
            ...
            continue
        if value in _SCHEMA_ACCEPTED_NOT_GENERATED_HASH_SET:
